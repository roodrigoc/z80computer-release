...
;
;   28C64 EEPROM  8kB  0000h - 1FFFh
;   62256 SRAM   32kB  2000h - 7FFFh
;   62256 SRAM   32kB  8000h - FFFFh
;
;
;   Compiler: Tasm
;
...
; =============================================================================
; --- Hardware Mapping ---
PORTX           .equ 01h ;PORTX address 
PORTA           .equ 02h ;PORTA address 
PORTB           .equ 04h ;PORTB address 
EN		.equ	01h				;LCD enable pin (PORTB bit 1)
RS		.equ	01h				;LCD RS pin (uses or logic)
...
px7:							;incrementa nibble mais significativo do endereço / seta dado com o valor 88h
		...
		cp		00h				;compara com 00h
		jp		nz,SET_88H		;se for 01h (escrita), desvia para SET_88H 
		ld		de,100h			;do contrário é modo leitura: carrega 100h no par de 
...
px6:							;incrementa segundo nibble do endereço / incrementa nibble mais significativo do dado 
		...
		ld		de,10h			;do contrário é modo leitura: carrega 10h no par de 
		add		hl,de 			;incrementa o par hl (endereço atual) em 10h 
		jp		final_kb		;desvia para saída da sub-rotina 
		
INC_DTH:						;incrementa dado HIGH 
		ld		a,(hl)			;carrega conteúdo do endereço atual em acc
		add		a,10h			;incrementa nibble mais significativo do dado 
...
px4:							;alterna entre o modo leitura/escrita 
		...
		xor		01h				;inverte bit menos significativo de acc
...
px3:							;decrementa nibble mais significativo do endereço / seta dado com o valor 00h 	
		...
		ld		de,100h			;carrega 100h no par de 
...
DEC_DTH:						;decrementa dado HIGH 		
		ld		a,(hl)			;carrega conteúdo do endereço atual em acc
		sbc		a,10h			;decrementa nibble mais significativo do dado 
		ld		(hl),a			;escreve na memória 
		jr		final_kb		;desvia para saída da sub-rotina
...
lcd_begin:
		...
		ld		b,30h			;protocolo de inicialização
		ld		c,00h			;envio de comando
		...
		ld		c,00h			;envio de comando
		call	send_nibble		;envia 30h para o LCD
		ld		b,20h			;LCD no modo 4 bits
		ld		c,00h			;envio de comando
		call	send_nibble		;envia 30h para o LCD
		ld		b,28h			;5x8 pontos por caractere, duas linhas
		call	lcd_cmd			;envia comando 28h
		ld		b,0Fh			;liga display, cursor e blink
		call	lcd_cmd			;envia comando 0Fh
		ld		b,01h			;limpa LCD
		call	lcd_cmd			;envia comando 01h
		ld		b,06h			;modo de incremento de endereço para direita, movendo apenas o cursor 
...
lcd_cmd:
		...
		jr		send_byte
lcd_write:
		ld		c,01h			;01h para envio de caracteres
send_byte:		
		...
		rla						;
		rla						;
		and		0F0h			;máscara para preservar nibble mais significativo
		ld		b,a				;atualiza b
...
; --- Limpa LCD ---
lcd_clear:
		ld		b,02h			;return home
		call	lcd_cmd			;envia 02h para o LCD
		ld		b,01h			;limpa o display
...
msg_init:
		ld		b,0C0h			;posiciona cursor na linha 1, coluna 0
		call	lcd_cmd			;envia comando 
		...
		ld		b,'0'
		call 	lcd_write 		
		ld		b,80h			;posiciona cursor na linha 0, coluna 0
		call	lcd_cmd			;envia comando 
		ret						;retorna da sub-rotina 
...
d100ms:							; 4.25µs  17 T States
		push	bc 				; 2.75µs  11 T States 
		ld		b,97h			; 1.75µs   7 T States 
aux1:
		ld		c,0BDh			; 1.75µs   7 T States 
aux2:
